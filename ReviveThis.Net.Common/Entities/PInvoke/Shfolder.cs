...
// ReSharper disable once InconsistentNaming
  ...
  public class ShLink
  {
    ...
    private readonly ShellLink _link;

    public ShLink(string fileName)
    {
      _fileName = fileName;

      _link = new ShellLink();

      ((IPersistFile) _link).Load(fileName, STGM_READ);
    }
    ...
    public string GetPath
    {
      get
      {
        if (!string.IsNullOrEmpty(_getPath))
          return _getPath;

        var sb = new StringBuilder(MAX_PATH);
        WIN32_FIND_DATAW data;
        ((IShellLinkW) _link).GetPath(sb, sb.Capacity, out data, SLGP_FLAGS.SLGP_UNCPRIORITY);
        return _getPath = /*string.IsNullOrEmpty(data.cFileName) ? string.Empty :*/ sb.ToString();
      }
    }
    #endregion
